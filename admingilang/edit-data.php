...
<body>
    ...
    <div class="content">
        <div class="header" style="background-color: green;">
            <h1>Edit Artikel Update</h1>
        </div>
        <hr>
        <br>
        <?php
        $id = $_GET['id'];
        $sql = mysqli_query($db_link, "SELECT * FROM artikel WHERE id='$id'");
    ...
    if (isset($_POST['proses'])) {
        ...
        if ($image != "") {
            ...
            if (in_array($ekstensi, $ekstensi_berhasil) === true) {
                ...
                $query = mysqli_query($db_link, "UPDATE artikel SET judul='$judul', tanggal='$tanggal', link='$link', image='$new_name' WHERE id='$id'");
            ...
            $query = mysqli_query($db_link, "UPDATE artikel SET judul='$judul', tanggal='$tanggal', link='$link' WHERE id='$id'");
